...
public class SystemTrayExample extends QWidget {
	...
	public static void main(String[] args) {
		...
		Ui_Settings setting = new Ui_Settings();
		setting.setupUi(new QDialog(){{show();}});
		QApplication.exec();
	}
	...
	public SystemTrayExample(QWidget parent) {
		...
		trayIcon.activated
				.connect(this,
						"activated(com.trolltech.qt.gui.QSystemTrayIcon$ActivationReason)");
		...
		QCheckBox toggleIconCheckBox = new QCheckBox(
	...
	protected void showMessage() {
		...
		if (QSysInfo.macVersion() != 0) {
			...
			icon = QSystemTrayIcon.MessageIcon
					.resolve(typeCombo.currentIndex());
	...
	public void activated(QSystemTrayIcon.ActivationReason reason) {
		String name = QSystemTrayIcon.MessageIcon.resolve(reason.value())
				.name();
		if (name != null)
